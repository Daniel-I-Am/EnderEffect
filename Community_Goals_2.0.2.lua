...
function finishedObjective()
	...
	displaySleepTime(3, 7200, textCol, secondaryCol, "Time till next objective:", "")
	os.execute("reboot")
end
...
function newGoal()
	completedObjective()
	displaySleepTime(4,timeLeft, textCol, secondaryCol, "Time left until next objective:", "")
	currentObjective = pickNewObjective()
	findItem()
	timeLeft = goalCompletionTime
end
...
function displaySleepTime(y,toSleep, txtcol, bgcol, textToShowPre, textToShowPost)
	repeat
		centerText("   " .. textToShowPre .. " " .. math.floor(toSleep/60) .. " minutes and " .. toSleep%60 .. " seconds" .. textToShowPost .. "   ", 1, w, height - 11, txtcol, bgcol)
		toSleep = toSleep - 1
		os.sleep(1)
	until toSleep == -1
end
