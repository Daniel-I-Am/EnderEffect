...
function completedObjective()
  if currentTier == 4 then throwError("Goal Completed") end
  if subObjective >= 4 then
    if currentTier ~= 4 then
      currentTier = currentTier + 1
    else
      throwError("Community Goal Completed !")
    end
    subObjective = 1
  else
...
function newGoal()
  ...
  currentObjective = pickNewObjective()
  findItem()
  displaySleepTime(4,4,timeLeft, "Time left until next objective:", "")
end
...
function displaySleepTime(x,y,toSleep, textToShowPre, textToShowPost)
  repeat
    gpu.set(x,y,textToShow .. " " .. math.floor(toSleep/60) .. " minutes and " .. toSleep%60 .. " seconds" .. textToShowPost)
    toSleep = toSleep - 1
    os.sleep(1)
  until toSleep == -1
end
...
function screen()
	...
	centerText("Deiver " .. currentObjective[2]*multiplier .. " of " .. currentObjective[1] .. " to our community chest!", 1, w, height - 12, textCol, secondaryCol)
