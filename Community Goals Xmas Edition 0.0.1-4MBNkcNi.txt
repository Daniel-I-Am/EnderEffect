transposer = {"dc3","26f","ded","3da"}
redstoneFlush = "cd7"
outputRedstoneIOs = {"bf6", "7b4", "27c", "7c7", "007"}
...
dataFile = "data.txt"
multiplier = 1
redstoneFlushSide = sides.east
redstoneOutputSide = sides.top
goalCompletionTime = 3600 --to solve
inbetweenGoalTime = 30 --completed a t4
betweenGoalTime = 1 --when completed
leftOffset = 20
...
function completedObjective()
	givePointToEveryoneOnline(currentTier)
...
function init()
	...
	findItem()
	givePointToEveryoneOnline(0)
	sortedNames, sortedCounts = sortArray(playerPoints, "d")
	term.clear()
end
...
function loadPoints()
	lineArray = {}
	playerPoints = {}
	debugLog("attempting to read file 'points.txt'")
	lineArray = fileRead("points.txt")
	debugLog("Succesfully read file")
	debugLog("")
	for i = 1, #lineArray do
		line = lineArray[i]
		local a = string.find(line, ":")
		local pointer = string.sub(line, 1, a-1)
		local value = string.sub(line, a+1)
		playerPoints[pointer] = value
		debugLog("Assigned '" .. value .. "' to '" .. pointer .. "' in 'PlayerPoints'")
	end
	for k,v in pairs(playerPoints) do print(k,v) end
	--event.pull("touch")
end

function givePointToEveryoneOnline(toAdd)
	lineArray = {}
	playerPoints = {}
	debugLog("attempting to read file 'points.txt'")
	lineArray = fileRead("points.txt")
	...
	debugLog("")
	onlinePlayers = debugCard.getPlayers()
	for i = 1, #onlinePlayers do debugLog("Player " .. onlinePlayers[i] .. " is online and has been found") end
	debugLog("")
	for i = 1, #onlinePlayers do
		player = tostring(onlinePlayers[i])
		currentValue = playerPoints[player]
		if currentValue == nil then currentValue = 0 end
		newValue = currentValue + toAdd
		debugLog("Player '" .. player .. "' now has '" .. newValue .. "' points")
		playerPoints[player] = newValue
	end
	debugLog("")
	if fs.exists("/home/points.txt") then fs.remove("/home/points.txt") end
	for k,v in pairs(playerPoints) do
		fileWrite("points.txt", k .. ":" .. v, false)
		debugLog("Written '" .. k .. ":" .. v .."' to file 'points.txt'")
	end
	debugLog("")
	debugLog("Finished data manipulation")
	sortedNames, sortedCounts = sortArray(playerPoints, "d")
end
...
function sortArray(array, mode)
	...
	if mode == "d" then
		for i = 0, #data do
			for j = 0, #data - i - 1 do
				local a = tonumber(data[j])
				local b = tonumber(data[j+1])
				local c = pointers[j]
...
function displaySleepTime(y,toSleep, txtcol, bgcol, textToShowPre, textToShowPost)
	repeat
		centerText("	 " .. textToShowPre .. " " .. math.floor(toSleep/60) .. " minutes and " .. toSleep%60 .. " seconds" .. textToShowPost .. "	 ",	1 + leftOffset, width, y, txtcol, bgcol)
		toSleep = toSleep - 1
		os.sleep(1)
	until toSleep == -1
end
...
function screen()
	...
	addRect(1 + leftOffset,1,width,height,primaryCol)
	addRect(8 + leftOffset,4,width-8,height-4,secondaryCol)
	gpu.setBackground(primaryCol)
	gpu.setForeground(textCol)
	addProgressBar(12 + leftOffset,height - 10,width - 12,height - 6,2,1,primaryCol,progressFilled,progressEmpty,0x000000,"",filledPercentage,true,false)
	
	centerText("COMMUNITY CHALLENGE", 1 + leftOffset, width, 5, textCol, secondaryCol)
	centerText("Work together to complete this challenge", 1 + leftOffset, width, 7, textCol, secondaryCol)
	centerText("And everyone online will be rewarded!", 1 + leftOffset, width, 9, textCol, secondaryCol)
	centerText("The current tier: " .. currentTier .. " - " .. subObjective, 1 + leftOffset, width, 11, textCol, secondaryCol)
	
	centerText("Deliver " .. currentObjective[2]*multiplier .. " of " .. currentObjective[1] .. " to our community chest!", 1 + leftOffset, width, height - 12, textCol, secondaryCol)
	centerText("Time remaining: " .. timeLeft, 1 + leftOffset, width, height - 11, textCol, secondaryCol)
	if storedAmount ~= nil then
		centerText("Current objective is at: ".. storedAmount .." / ".. currentObjective[2]*multiplier, 1 + leftOffset, width, height-5, textCol, secondaryCol)
	end
	addRect(1,1,leftOffset,height,primaryCol)
	if sortedNames~= nil then
		for i = 0, math.min(height-8,#sortedNames) do
			gpu.set(1,i+4,i+1 ..  ". " .. sortedNames[i] .. ": " .. sortedCounts[i])
		end
	end
end
